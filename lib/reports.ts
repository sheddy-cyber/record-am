...
export function generateReceiptHTML(sale: Sale, business: Business, branch: Branch): string {
  ...
  const statusColor =
    sale.payment_status === 'paid' ? '#238B5B' :
    sale.payment_status === 'partial' ? '#C87A22' :
    '#C44536';
  const statusLabel =
    sale.payment_status === 'paid' ? 'PAID IN FULL' :
...
<div class="report">
  ...
  <div class="section">
    ...
    <div class="row">
      <span>${index + 1}. ${product.name} <span style="color:#7D877F;font-size:12px;">(×${product.qty})</span></span>
